...
typedef struct {
  ...
  size_t num_events;
  KMD_EVENT* events;
  uint8_t tag_title[ KMD_MAX_MESSAGE_LEN + 2 ];
  uint8_t tag_artist[ KMD_MAX_MESSAGE_LEN + 2 ];
  uint8_t tag_album[ KMD_MAX_MESSAGE_LEN + 2 ];
  uint8_t tag_artwork[ KMD_MAX_MESSAGE_LEN + 2 ];
  uint8_t tag_remote_path[ KMD_MAX_MESSAGE_LEN + 2 ];
} KMD_HANDLE;
